...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Leave}.
 */
@RestController
@RequestMapping("/api/leaves")
public class LeaveResource {

    private static final Logger log = LoggerFactory.getLogger(LeaveResource.class);
    ...
    public ResponseEntity<LeaveDTO> createLeave(@Valid @RequestBody LeaveDTO leaveDTO) throws URISyntaxException {
        ...
        leaveDTO = leaveService.save(leaveDTO);
        return ResponseEntity.created(new URI("/api/leaves/" + leaveDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, leaveDTO.getId().toString()))
            .body(leaveDTO);
    }
    ...
    public ResponseEntity<LeaveDTO> updateLeave(
        ...
        leaveDTO = leaveService.update(leaveDTO);
    ...
    public ResponseEntity<List<LeaveDTO>> getAllLeaves(
        ...
        Page<LeaveDTO> page;
        if (eagerload) {
            page = leaveService.findAllWithEagerRelationships(pageable);
        } else {
            page = leaveService.findAll(pageable);
        }
